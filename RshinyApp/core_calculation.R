...
safe_GET <- function(...) {
  tryCatch(
    ...
    error = function(e) {
      stop("UniProt API GET failed: ", e$message, call. = FALSE)
    }
  )
}
...
safe_POST <- function(...) {
  tryCatch(
    ...
    error = function(e) {
      stop("UniProt API POST failed: ", e$message, call. = FALSE)
    }
  )
}
...
isJobReady <- function(jobId) {
  ...
  for (i in seq_len(nTries)) {
    ...
    r <- tryCatch(
      safe_GET(
        paste0("https://rest.uniprot.org/idmapping/status/", jobId),
        accept_json()
      ),
      error = function(e) {
        stop("UniProt status polling failed", call. = FALSE)
      }
    )
  ...
  stop(
    "UniProt mapping timed out after ",
    pollingInterval * nTries,
    " seconds",
    call. = FALSE
  )
}
...
uniprot_map_ids <- function(ids,
  ...
  # Get redirect
  rdet <- safe_GET(
    paste0("https://rest.uniprot.org/idmapping/details/", jobId),
    accept_json()
  )
  ...
  df <- tryCatch(
    read.table(
      text = content(rr, "text"),
      sep = "\t",
      header = TRUE
    ),
    error = function(e) {
      stop("Failed to parse UniProt TSV response", call. = FALSE)
    }
  )
